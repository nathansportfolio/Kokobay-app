...
import { fetchKokobayJson, isKokobayWebProductsConfigured, tryFetchKokobayJson } from './client';
...
export async function fetchKokobayPredictiveSearch(
  ...
  const data = await tryFetchKokobayJson(`/api/search/predictive?${params.toString()}`);
